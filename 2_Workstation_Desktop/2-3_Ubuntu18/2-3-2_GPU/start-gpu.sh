...
apt-get install -y xfsprogs ntfs-3g aptitude lvm2 dstat curl git-lfs figlet >> /root/log.txt
...
echo -e  "${PROMPT} sed -i '1a bash /root/LISR/2_Workstation_Desktop/2-3_Ubuntu18/2-3-2_GPU/Dasan_Ubuntu18-Standard-install.sh >> /root/dasan_install_log.txt' /etc/rc.local   " >> /root/dasan_install_log.txt
