...
echo ""
echo -e  "\033[1;34m${PROMPT} sed -i '1a bash /root/LISR/2_Workstation_Desktop/2-5_Ubuntu20/2-5-1_CPU/Dasan_Ubuntu20-Standard-install.sh >> /root/dasan_install_log.txt' /etc/rc.local   \033[0m" >> /root/dasan_install_log.txt
sed -i '1a bash /root/LISR/2_Workstation_Desktop/2-5_Ubuntu20/2-5-1_CPU/Dasan_Ubuntu20-Standard-install.sh >> /root/dasan_install_log.txt' /etc/rc.local
