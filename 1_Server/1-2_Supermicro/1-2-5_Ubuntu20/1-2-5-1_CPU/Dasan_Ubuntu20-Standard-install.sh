...
apt-get install -y vim nfs-common rdate xauth firefox gcc make locate htop tmux git wget figlet
...
echo -e  "${PROMPT} apt update " >> /root/dasan_install_log.txt
apt update
...
echo -e  "${PROMPT} which pip " >> /root/dasan_install_log.txt
which pip
...
echo -e  "${PROMPT} cat /usr/local/bin/pip " >> /root/dasan_install_log.txt
...
echo ""
echo -e  "${PROMPT} bash /root/LISR/1_Server/1-2_Supermicro/1-2-5_Ubuntu20/1-2-5-2_GPU/Install_Dell_MSM_Ubuntu.sh " >> /root/dasan_install_log.txt
bash /root/LISR/1_Server/1-2_Supermicro/1-2-5_Ubuntu20/1-2-5-2_GPU/Install_Dell_MSM_Ubuntu.sh >> /root/log.txt
