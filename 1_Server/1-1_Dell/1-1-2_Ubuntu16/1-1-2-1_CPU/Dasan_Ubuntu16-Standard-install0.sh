...
apt-get install -y xfsprogs ntfs-3g aptitude lvm2 dstat curl smartmontools figlet >> /root/log.txt

sleep 5

echo ""
echo -e  "${PROMPT} DEBIAN_FRONTEND=noninteractive apt-get install -y mailutils " >> /root/dasan_install_log.txt

DEBIAN_FRONTEND=noninteractive apt-get install -y mailutils >> /root/log.txt
...
echo -e  "${PROMPT} apt-get install -y python-dev python3-dev python-pip python3-pip python-tk python3-tk " >> /root/dasan_install_log.txt
apt-get install -y python-dev python3-dev python-pip python3-pip python-tk python3-tk  >> /root/log.txt
...
echo -e  "${PROMPT} sed -i '14s/Dasan_Ubuntu16-Standard-install0.sh/Dasan_Ubuntu16-dell-package-install.sh/g' /etc/rc.local    " >> /root/dasan_install_log.txt
sed -i '14s/Dasan_Ubuntu16-Standard-install0.sh/Dasan_Ubuntu16-dell-package-install.sh/g' /etc/rc.local
