...
echo "  기본 유틸 설치. "
echo -e  "\033[1;34m${PROMPT} apt-get install -y vim nfs-common rdate xauth firefox gcc make net-tools \033[0m" >> /root/dasan_install_log.txt
...
echo -e  "\033[1;34m${PROMPT} sed -i '1a bash /root/LISR/1_Server/1-1_Dell/1-1-3_Ubuntu18/1-1-3-1_CPU/Dasan_Ubuntu18-Standard-install0.sh >> /root/dasan_install_log.txt' /etc/rc.local   \033[0m" >> /root/dasan_install_log.txt
sed -i '1a bash /root/LISR/1_Server/1-1_Dell/1-1-3_Ubuntu18/1-1-3-1_CPU/Dasan_Ubuntu18-Standard-install0.sh >> /root/dasan_install_log.txt' /etc/rc.local
