...
echo -e  "\033[1;34m${PROMPT} sed -i '1a bash /root/LISR/1_Server/1-2_Supermicro_Etc/1-2-3_Ubuntu18/1-2-3-1_CPU/Dasan_Ubuntu18-Standard-install0.sh >> /root/dasan_install_log.txt' /etc/rc.local   \033[0m" >> /root/dasan_install_log.txt
sed -i '1a bash /root/LISR/1_Server/1-2_Supermicro_Etc/1-2-3_Ubuntu18/1-2-3-1_CPU/Dasan_Ubuntu18-Standard-install0.sh >> /root/dasan_install_log.txt' /etc/rc.local
