...
echo ""
echo -e  "\033[1;34m${PROMPT} sed -i '1a bash /root/LISR/1_Server/1-3_Qct/1-3-5_Ubuntu20/1-3-5-1_CPU/Dasan_Ubuntu20-Standard-install.sh >> /root/dasan_install_log.txt' /etc/rc.local   \033[0m" >> /root/dasan_install_log.txt
sed -i '1a bash /root/LISR/1_Server/1-3_Qct/1-3-5_Ubuntu20/1-3-5-1_CPU/Dasan_Ubuntu20-Standard-install.sh >> /root/dasan_install_log.txt' /etc/rc.local
