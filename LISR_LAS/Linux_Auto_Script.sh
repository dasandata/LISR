...
then
  ...
  echo "" | tee -a /root/install_log.txt
  echo "CUDA Version is already" | tee -a /root/install_log.txt
fi
...
if [ $? != 0 ]
  then
    ...
    echo "" | tee -a /root/install_log.txt
    echo "rc.local Setting" | tee -a /root/install_log.txt
    case $OSCHECK in
      centos )
...
# rc.local 기본 값으로 변경
  echo "" | tee -a /root/install_log.txt
  echo "Complete auto-script setup" | tee -a /root/install_log.txt
    case $OS in
      centos7 | centos8 )
        sed -i '/root/d' /etc/rc.d/rc.local
        systemctl set-default graphical.target >> /root/install_log.txt 2> /root/log_err.txt
...
" >>  /etc/crontab
  ...
  echo "" | tee -a /root/install_log.txt
  echo "Server Temperature setting Error" | tee -a /root/install_log.txt
fi
...
### 스크립트 완료 후 필요없는 파일 목록 (테스트 진행하며 추가 및 삭제 여부 진행 예정)
## rm -f customername.txt cudaversion.txt nvidia.txt
## centos 는 nvidia-machine-learning-repo-rhel8-1.0.0-1.x86_64.rpm , cuda-repo-rhel8-10.2.89-1.x86_64.rpm 추가 삭제
## customername.txt / 고객사 이름 변수 저장 파일
## cudaversion.txt / 쿠다 버전 선택 변수 저장 파일
## install_log.txt / 모든 기록 저장 파일
## log_err.txt / 실행 중 에러 메시지 저장 파일
## nvidia.txt / GPU 존재 여부 확인 파일
## hwcheck.txt / 하드웨어 정보 파일
## cuda-repo-rhel8-10.2.89-1.x86_64.rpm / 쿠다 저장소 설정시 다운받은 파일
## nvidia-machine-learning-repo-rhel8-1.0.0-1.x86_64.rpm / 쿠다 저장소 설정시 다운받은 파일
